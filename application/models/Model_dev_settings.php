...
<?php
    ...
    public function enable_disable_aboutus($data){
        $sql = "UPDATE sys_aboutus SET status = ? WHERE id = ?";
        $bind_data = array(
            $data['status'],
            $data['selectedid']
        );
    ...
    public function delete_aboutus($data){
        $sql = "UPDATE sys_aboutus SET status = 0 WHERE id = ?";
        $bind_data = array(
            $data['selectedid']
        );
        return $this->db->query($sql,$bind_data);
    ...
    public function save_aboutus($data){
        $count = $this->db->query("SELECT * FROM sys_faqs where status > 0")->num_rows();
        if($data['selectedid']==''){
            $sql = "INSERT INTO sys_aboutus(title,content,arrangement,status) VALUES(?,?,?,?)";
            $bind_data = array(
                $data['f_title'],
                $data['f_content'],
                $count+1,
            ...
            $sql = "UPDATE sys_aboutus SET title = ?,content = ? WHERE id = ?";
            $bind_data = array(
                $data['f_title'],
                $data['f_content'],
                $data['selectedid']
    ...
    public function get_aboutus($id){
        $sql = "SELECT * from sys_aboutus WHERE `id` = $id";
        return $this->db->query($sql)->result_array();
    }
    public function get_active_aboutus(){
        $sql = "SELECT * from sys_aboutus WHERE `status` = 1 ORDER BY arrangement";
        return $this->db->query($sql)->result_array();
    }
    ...
    public function load_aboutus($requestData,$exportable = false) {
		// storing  request (ie, get/post) global array to a variable  
		$date_created       = $this->input->post('date_created');
		$_record_status     = $this->input->post('_record_status');
		$_search 			= $this->input->post('_search');
		...
		foreach( $query->result_array() as $row ) {  // preparing an array for table tbody
			...
			if($this->loginstate->get_access()['web']['update'] == 1){
				$buttons .= '<div class="dropdown-divider"></div>
				<button class="dropdown-item update_aboutus" data-value="'.$row['id'].'" data-toggle="modal" data-target="#add_modal"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</button>';
			}
    ...
    public function save_arrangement_aboutus($id,$order){
        $sql = "UPDATE sys_aboutus set arrangement = $order WHERE `id` = $id";
