$(document).ready(function () {
	...
	function gen_order_history_tbl(search, id, total_amount) {
		var order_history_tbl = $("#history_tbl").DataTable({
			...
			ajax: {
				url: base_url + "customers/Main_customers/get_order_history",
				type: "post",
				data: {
					searchValue: search,
					id,
	...
	function gen_login_history_tbl(id) {
		var order_history_tbl = $("#login_history_tbl").DataTable({
			...
			order: [[2, "desc"]],
			ajax: {
				url: base_url + "customers/Main_customers/get_login_history",
				type: "post",
				data: {
					id: id,
				},
	...
	function fillDatatable() {
		...
		var dataTable = $("#table-grid").DataTable({
			...
			ajax: {
				type: "post",
				url: base_url + "customers/Main_customers/get_customers", // json datasource
				data: {
					_type: _type,
					_name: _name,
					_city: _city,
	...
	$("#search_hideshow_btn").click(function (e) {
		...
		if (!visibility) {
			...
			$("#search_hideshow_btn").html(
				'&ensp;Hide Search <i class="fa fa-chevron-circle-down fa-lg" aria-hidden="true"></i>'
			);
		} else {
			//not visible
			$("#search_hideshow_btn").html(
				'Show Search <i class="fa fa-chevron-circle-up fa-lg" aria-hidden="true"></i>'
			);
		}
	...
	$("#addBtnCustomer").click(function (e) {
		...
		if (
			...
			$.ajax({
				type: "post",
				url: base_url + "customers/Main_customers/add_customer",
				data: {
					_fname: _fname,
					_lname: _lname,
	...
	$.ajax({
		type: "post",
		url: base_url + "customers/Main_customers/get_cities",
		success: function (data) {
