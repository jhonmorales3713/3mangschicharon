$(function(){
  ...
  $("#search_clear_btn").click(function(e){
    ...
    $('#reprange option[value="today"]').prop('selected',true).trigger('change');
    chart();
		gen_chart_tbl();
	});
  ...
  if(transactions_chart){
 	...
 	function fill_data(data,member_id){
		...
		if(transactions_chart)
        {
            ...
            {
	            if(res.length > 0){
	                trandate = res.map(function(obj) { return obj['trandate']; });
	                //data0 = res.map(function(obj) { return obj['totalamount']; });
	                data0 = res.map(function(obj) { return obj['netamount']; });
	                //totalamount    = res.map(function(obj) { return parseFloat(obj['totalamount']); }).reduce(function(total,num) { return total+num; });
	                totalnet    = res.map(function(obj) { return parseFloat(obj['netamount']); }).reduce(function(total,num) { return total+num; });
				}
	            else
	            {
	            	trandate = res.map(function(obj) { return obj['trandate']; });
	                //data0 = res.map(function(obj) { return obj['totalamount']; });
	                data0 = res.map(function(obj) { return obj['totalnet']; });
	            }
			}
