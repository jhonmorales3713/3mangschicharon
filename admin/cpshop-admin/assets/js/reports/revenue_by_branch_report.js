$(function(){
    ...
    var element = document.getElementById("rbb_chart");
    var rbb_chart = new Chart(element, {
        type: 'horizontalBar',
        data: {
            labels: [],
            datasets: [{
    ...
    function fill_data(rBB)
    {
        if(rbb_chart){
            rbb_chart.data.labels = rBB.branchnames;
            // console.log(v);
            rbb_chart.data.datasets[0].data = rBB.data1;
            rbb_chart.data.datasets[0].label = rBB.legend[0];
            rbb_chart.data.datasets[1].data = rBB.data2;
            rbb_chart.data.datasets[1].label = rBB.legend[1];
            rbb_chart.update();
        }
    }
    ...
    function table(){
      $('#table-grid').DataTable( {
        ...
        "columnDefs": [
          ...
          {"targets":[1,4,5],"visible":(filter.filtertype == 'logs') ? true:false},
          {"targets":[6],"visible":(filter.filtertype == 'logs') ? false:true}
        ],
        "ajax":{
          url: base_url+'reports/revenue_by_store/rbb_data',
          type: 'post',
          data: filter,
          beforeSend: function(){
            $.LoadingOverlay("show");
    ...
    function chart(){
        $.ajax({
            type:'post',
            url: base_url+'reports/revenue_by_store/rbb_chart_data',
            data: filter,
