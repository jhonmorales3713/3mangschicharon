$(function(){
    ...
    var element = document.getElementById("rbl_chart");
    var rbl_chart = new Chart(element, {
        type: 'pie',
        data: {
            labels: [],
            datasets: []
    ...
    function table(){
      $('#table-grid').DataTable( {
        ...
        "order":[[0, 'asc']],
        "columnDefs": [
          {"targets":[1],"visible":(filter.filtertype == 'logs') ? true:false},
          {"targets":[1,2],"visible":(filter.shopid > 0 && filter.filtertype == 'logs') ? true:false},
        ],
        "ajax":{
          url: base_url+'reports/revenue_by_store/rbl_data',
          type: 'post',
          data: filter,
          beforeSend: function(){
            $.LoadingOverlay("show");
    ...
    function chart(){
        $.ajax({
            type:'post',
            url: base_url+'reports/revenue_by_store/rbl_chart_data',
            data: filter,
