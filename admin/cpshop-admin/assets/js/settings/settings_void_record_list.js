$(function () {
	...
	function fillDatatable() {
		...
		var dataTable = $('#table-grid').DataTable({
			...
			"serverSide": true,
			responsive: true,
			columnDefs: [{targets: [6], orderable: false, sClass: "text-center"}, { responsivePriority: 1, targets: 0 }],
			// "columnDefs": [
			// 	{ targets: 5, orderable: false, "sClass":"text-center"}
			// ],
			"ajax": {
				type: "post",
				url: base_url + "settings/void_record/Settings_void_record/void_record_table", // json datasource
				data: { '_name': _name, 'status': status, 'date_from': date_from, 'date_to': date_to, order_status_view: order_status_view }, // serialized dont work, idkw
