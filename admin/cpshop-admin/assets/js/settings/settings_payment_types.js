$(function () {
	...
	function fillDatatable() {
		...
		var _payment_type = $("input[name='_payment_type']").val();
		...
		var dataTable = $('#table-grid').DataTable({
			...
			"ajax": {
				type: "post",
				url: base_url + "settings/payment_types/payment_types_table", // json datasource
				data: {
					'_record_status': _record_status,
					'_code': _code,
					'_payment_type': _payment_type,
	...
	$('#add_record_form').submit(function (e) {
		e.preventDefault();
		$('#addPaymentType').attr('disabled', true);
		const form_data = $(this).serializeArray();
		if (validateInputs(form_data)) {
			$.ajax({
				type: "post",
				url: base_url + "settings/payment_types/create_data",
				data: form_data,
				success: function (data) {
					if (data.success == 1) {
						...
						$('#addPaymentType').attr('disabled', false);
					}
					else {
	...
	$('#table-grid').delegate(".action_edit", "click", function () {
		...
		$.ajax({
			type: 'post',
			url: base_url + 'settings/payment_types/get_data',
			data: { 'id': id },
			success: function (data) {
				const res = data[0];
				prev_val = {
					'code' : res.paycode,
					'payment' : res.description
				};
				$("input[name='_edit_code']").val(res.paycode);
				$("input[name='_edit_payment']").val(res.description);
			}
		});
	});
	...
	$('#edit_record_form').submit(function (e) {
		...
		form_data.push({ name: 'prev_val', value: JSON.stringify(prev_val) });
		// console.log(form_data);
		if (validateInputs(form_data)) {
			$.ajax({
				type: "post",
				url: base_url + "settings/payment_types/update_data",
				data: form_data,
				success: function (data) {
					if (data.success == 1) {
						fillDatatable();
	...
	$("#disable_modal_confirm_btn").click(function (e) {
		...
		$.ajax({
			type: 'post',
			url: base_url + 'settings/payment_types/disable_data',
	...
	$("#delete_modal_confirm_btn").click(function (e) {
		...
		$.ajax({
			type: 'post',
			url: base_url + 'settings/payment_types/delete_data',
