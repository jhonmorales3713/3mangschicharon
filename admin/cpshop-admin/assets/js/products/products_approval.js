				...
				bootbox.confirm({
					...
					callback: function (result) {
						if(result == true){
							...
							var url = base_url+"products/Products_approval/product_waiting_for_approval_application_all";
							$.post(url,{ product : addProdArr },function(rs){
								...
								if(rs.success){
									...
									showCpToast("success", "Approved!", "Product changes has been approved.");
					...
					callback: function (result) {
						if(result == true){
							...
							//   alert(textarea);
								if(textarea != ''){
										$.LoadingOverlay("show");
										var url = base_url+"products/Products_approval/product_waiting_for_approval_decline_all";
...
function load_waiting_for_Approval(){
	var base_url = $("body").data('base_url'); //base_url came from built-in CI function base_url();
		...
		$.ajax({
			type: "post",
			url:base_url+"products/Products_approval/product_waiting_for_approval_table", // json datasource
...
function Display_waiting_for_Approval_Table(result)
	{	
        ...
        {  
			...
			console.log('test',resultdata['itemname']);
		
		  if(resultdata['parentIDproduct'] != null && resultdata['parentIDproduct']  != 0){
				row = "";

				row += "<tr class='product_tr_"+resultdata['Id']+"'>";
				row += "<td class='product_tr_"+resultdata['Id']+"'><input type='checkbox' class='checkbox_perprod' id='checkbox_perprod' name='checkbox_perprod' value='"+ resultdata['Id'] +"' /></td>";
			...
			if(parseInt(Merchant_Comrate) == parseInt('0') &&  parseInt(start_up) == parseInt('0') &&  parseInt(jc) == parseInt('0') &&  parseInt(mcjr) == parseInt('0') &&  parseInt(mc) == parseInt('0') &&  parseInt(mcsuper) == parseInt('0') &&  parseInt(mcmega) == parseInt('0') &&   parseInt(others) == parseInt('0')){
			   ...
			   row += "<td class='product_tr_"+resultdata['Id']+"'>"+resultdata['approval_price']+"</td>";
				...
				$buttons = "";
				   $buttons += '<a class="btn btn-primary" data-prod-id="' +resultdata['Id']+ '" href="'+base_url+('Products_approval/view_products_approval/' + token + '/' + resultdata['Id']) + '"> View</a> &nbsp;';
				...
				if(access_edit == 1){
					$buttons += '<a class="btn btn-success"  href="'+base_url+('Main_products/update_variants/' + token + '/' + resultdata['Id'] + '/' + resultdata['parent_product_id']) + '"> Edit </a> &nbsp;';
				}
		  ...
		  if(parseInt(Merchant_Comrate) == parseInt('0') &&  parseInt(start_up) == parseInt('0') &&  parseInt(jc) == parseInt('0') &&  parseInt(mcjr) == parseInt('0') &&  parseInt(mc) == parseInt('0') &&  parseInt(mcsuper) == parseInt('0') &&  parseInt(mcmega) == parseInt('0') &&   parseInt(others) == parseInt('0')){
			...
			if(access_edit == 1){
			    $buttons += '<a class="btn btn-success"  href="'+base_url+('Main_products/update_products/' + token + '/' + resultdata['Id']) + '"> Edit </a> &nbsp;';
	     	}
			row += "<td class='product_tr_"+resultdata['Id']+"'>"+$buttons+"</td>";
			row += "</tr>";
