$(function () {
  ...
  var orderable = {};
  if(access != 'clc1ZkhIZnlSMXFoTUFWODE0eXkyUT09'){ // admin view = 1
    if(ini == 'c0Nta1MvSVRTZ2s3anFaVW9ZQ2tiUT09'){ // ini == toktokmall
      main_visible = { targets: [3], visible: false };
      target_visible = {targets: [3,6,7,8,10], visible: false};
      target_visible_logs = { targets: [3,4,8], visible: false };
      orderable = { targets: [10, 11], orderable: false };
    }else{
      main_visible = { targets: [3], visible: false };
      target_visible = {targets: [3,5,6,8], visible: false};
      target_visible_logs = { targets: [2,3], visible: false };
      orderable = { targets: [5, 7, 8, 10, 11], orderable: false };
    }
  }
  ...
  function get_billing_table(search) {
    var billing = $('#table-grid').DataTable({
      ...
      "ajax": {
        url: base_url + 'accounts/Billing/get_billing_table',
        type: 'post',
        data: {
          searchValue: search
        },
  ...
  function get_billing_breakdown_table(billing_data) {
    var billing = $('#table-item').DataTable({
      ...
      "order": [[0, 'desc']],
      "columnDefs": [
        { targets: [5, 7, 8, 10, 11], orderable: false },
        target_visible
      ],
      "ajax": {
        url: base_url + 'accounts/Billing/get_billing_breakdown_table',
        type: 'post',
        data: billing_data,
        beforeSend: function () {
          $.LoadingOverlay("show");
  ...
  function gen_branch_billing_tbl(search, id, trandate) {
    var table_branch = $('#table-item-branch').DataTable({
      ...
      "order": [[0, 'desc']],
      "ajax": {
        url: base_url + 'accounts/Billing/get_billing_branch_tbl',
        type: 'post',
        data: {
          searchValue: search,
          shopid: id,
  ...
  function get_billing_logs(order_id, totalamount, processfee, netamount, total_comrate) {
    var billing = $('#tbl_logs').DataTable({
      ...
      "ajax": {
        url: base_url + 'accounts/Billing/get_billing_logs',
        type: 'post',
        data: {
          order_id: order_id,
          totalamount, processfee, netamount, total_comrate
        },
        beforeSend: function () {
          $.LoadingOverlay("show");
        },
  ...
  function get_billing_branch_logs(branchid, trandate, totalamount, processfee, netamount) {
    var billing = $('#tbl_branch_logs').DataTable({
      ...
      "order": [[0, 'desc']],
      "ajax": {
        url: base_url + 'accounts/Billing/get_billing_branch_logs',
        type: 'post',
        data: {
      ...
      success: function (json_data) {
        ...
        if (json_data.success == true) {
          ...
          if (json_data.message.paystatus != 'Settled') {
            ...
            if(json_data.message.deposit_date == "" || json_data.message.deposit_date == null){
              $('#tm_depositdate').html(json_data.message.paiddate);
            }else{
              $('#tm_depositdate').html(json_data.message.deposit_date);
            }
  ...
  $(document).on('click', '.btn_view_logs', function () {
    ...
    let total_actual = $(this).data('total_actual');
    let total_srp = $(this).data('total_srp');
    let processfee = $(this).data('processfee');
    let netamount = $(this).data('netamount');

    get_billing_logs(orderid, total_actual, processfee, netamount, total_comrate);
    $('#view_modal').modal();
  });
  ...
  $(document).on('click', '#btn_manual_trigger', function(){
    $('#manual_cron_modal').modal();
  });
