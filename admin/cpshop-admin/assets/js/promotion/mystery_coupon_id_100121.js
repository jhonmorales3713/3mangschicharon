$(function(){
	...
	$('#savePromoConfirm').click(function(e){
		...
		rate 	   = $('#batch_promo_rate').val();
		price 	   = $('#batch_promo_price').val();
        form_data.append('deletedProductArr', deletedProductArr);
		const formData = $('#form_promoprod').serializeArray();
        ...
        if(save == 2){
			...
			//sys_toast_warning('Perentage should not be higher than 100.');
			showCpToast("warning", "Warning!", 'Perentage should not be higher than 100.');
			$.LoadingOverlay("hide");
            ...
            $.ajax({
				...
				data:{
                    ...
                    'deletedProductArr': JSON.stringify(deletedProductArr),
                },
                // contentType: false,   
                // cache: false,      
                // processData:false,
	...
	function populateProduct2(){
		...
		var dataTable = $('#table-grid-productpromo').DataTable({
			...
			"ajax":{
				type: "post",
				url:base_url+"promotion/Main_promotion/mc_product_table", // json datasource
				data: {
	...
	function getCampaignList(){
		$.ajax({
            type:'post',
            url:base_url+'promotion/Main_promotion/get_campaign_type_list',
            async:false,
            success:function(data){
                $.LoadingOverlay("hide"); 
                var json_data = JSON.parse(data);
                if(json_data.success){
                	campaignArray = [];
					//counter = 0;
					$.each(json_data.campaignArray, function(key, value) {
						dataArr = {
							'campaign_id' : value.id,
							'campaign_name' : value.name
						};
						campaignArray.push(dataArr);
	...
	let total_row = 0;
	const textTotalrow = document.getElementById("total_row");
	...
	function displayProductPromotion(value, key){
		...
		if(value.product_id != null && value.product_name != null && value.product_id && 'null' && value.product_name != 'null'){
			...
			$.each(campaignArray, function(key, row) {

				if(row.campaign_id == value.product_promo_type){
					//console.log(value.product_promo_type)
			   		select_promo_type += "<option selected value='"+row.campaign_id+"'>"+row.campaign_name+"</option>";
				}
			});
			...
			var new_promo_price = value.product_promo_price
			if(value.product_promo_rate != 1){
				new_promo_price = parseInt(value.product_promo_price);
			}
			...
			select_purch_limit = "<select id='select_purch_limit' readonly class='form-control mb-2 select_purch_limit"+value.product_id+"' data-value='"+value.product_id+"'>";
			...
			str += "<td class='product_tr_"+value.product_id+"' width='5%'><input type='checkbox' id='checkbox_prod' class='form-control checkbox_prod' name='checkbox_prod' value='"+value.product_id+"'></td>";
			...
			str += "<td class='product_tr_"+value.product_id+"' width='15%'><input type='text' min='0' class='form-control allownumericwithdecimal notallowzero product_promo_price"+value.product_id+"' name='product_promo_price[]' onkeypress='return isNumberKey(event)' style='background-color:white;' value="+new_promo_price+"></td>";
		...
		textTotalrow.innerHTML = total_row;
	...
	function loadExistingData(){
		$.ajax({
            type:'post',
            url:base_url+'promotion/Main_promotion/mc_fetch_productPromo_id',
            data:{
            ...
            success:function(data){
   			...
   			// 			end_date = ((date.getMonth() > 8) ? (date.getMonth() + 1) : ('0' + (date.getMonth() + 1))) + '/' + ((date.getDate() > 9) ? date.getDate() : ('0' + date.getDate())) + '/' + date.getFullYear();
						...
						$('#start_date').val(value.startdate);
						$('#end_date').val(value.enddate);
						...
						set_selected('batch_promo_type', value.promo_type);
						set_selected('batch_promo_rate', value.promo_rate);

						$("#batch_promo_stock_qty").val(parseInt(value.promo_stock));
						$("#batch_purch_limit").val(value.purchase_limit);

						var new_promo_price = value.promo_price
						if(value.promo_rate != 1){
							new_promo_price = parseInt(value.promo_price);
						}

						$("#batch_promo_price").val(new_promo_price);

						alignProductPromotion(dataArr2, counter);
						counter++;
						productArray.push(dataArr2);
						//productIdArray.push(dataArr2);
					});
					prependProductTable();
					$.LoadingOverlay("hide");
                }else{
...
let count = 0;
const textHolder = document.getElementById("number_of_check");
textHolder.innerHTML = count;

$(document).delegate('#checkbox_prod','change',function(e){
    if ($(this).is(":checked")) {
    	textHolder.innerHTML = ++count;
   ...
   //$("#number_of_check").append(count_check);
});
