$(function(){
	...
	function displayProductPromotion(value, key){
		...
		if(value.product_id != null && value.product_name != null && value.product_id && 'null' && value.product_name != 'null'){
			...
			$.each(campaignArray, function(key, value) {
				//console.log(value.campaign_id)
			   select_promo_type += "<option value='"+value.campaign_id+"'>"+value.campaign_name+"</option>";
			});
			...
			str += "<td class='product_tr_"+value.product_id+"' width='15%'><input type='text' min='0' class='form-control allownumericwithdecimal notallowzero product_promo_price"+value.product_id+"' name='product_promo_price[]' onkeypress='return isNumberKey(event)' style='background-color:white;' value="+value.product_promo_price+"></td>";
	...
	// 	$.ajax({
 //            type:'post',
 //            url:base_url+'promotion/Main_promotion/mc_fetch_productPromo',
 //            data:{
 ...
 //                if(json_data.success){
	...
	// 					  //console.log(json_data.productArr);
