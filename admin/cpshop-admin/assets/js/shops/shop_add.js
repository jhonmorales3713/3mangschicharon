...
$('body').delegate("#check_rabutton_whatsnew", "click", function(e){
    ...
    e.preventDefault();
        $.ajax({
            ...
            success:function(data){
                var res = data.result;
                if (data >= 16){
                    showCpToast("info", "Note!", 'You have reached the maximum of 16 What`s New Merchants allowed');
                    // $.toast({
    ...
    e.preventDefault();
        $.ajax({
            ...
            success:function(data){
                var res = data.result;
                if (data >= 4){
                    showCpToast("info", "Warning!", 'You have reached the maximum of 4 featured merchants allowed'); 
                    // $.toast({
...
$('#entry-feat-merchant-arrangement').change(function(){ 
    var merchant_number = $(this).val();
    check_featuredproduct(merchant_number)
});

function check_featuredproduct(merchant_number)
{
    var base_url     = $("body").data('base_url'); //base_url came from built-in CI function base_url();
...
var edit_loadstate = false;
    $('#entry-form').submit(function(e){
        ...
        form_data.append('file_container_whatsnew', get_img_file('file_container_whatsnew'));
