...
class Model_orders extends CI_Model {
	...
	public function orders_details($ref_num, $sys_shop) {
		...
		if($sys_shop == 0) {
			$query=" SELECT a.*, a.order_id as orderid, a.total_amount  as totamt, a.order_status as orderstatus, a.delivery_amount as sf, a.payment_status as paystatus,a.order_type,
			...
			$query="SELECT * FROM (
					SELECT a.*, c.id as orderid, SUM(b.total_amount) as totamt, a.order_status as orderstatus, c.order_status as sales_order_status, s.delivery_amount as sf, 
						s.original_shipping_fee as osf, s.handle_shipping_promo as hsp, s.sys_shop as sys_shop, a.payment_status as paystatus, c.date_shipped as dateshipped,
						...
						FROM app_order_details as a
						LEFT JOIN `app_order_logs` as b ON a.order_id = b.order_id AND b.status > 0
					...
					UNION ALL
					SELECT a.*, c.id as orderid, SUM(b.total_amount) as totamt, a.order_status as orderstatus, c.order_status as sales_order_status, s.delivery_amount as sf,
						s.original_shipping_fee as osf, s.handle_shipping_promo as hsp, s.sys_shop as sys_shop, c.payment_status as paystatus, c.date_shipped as dateshipped,
						s.daystoship as paid_daystoship, s.daystoship_to as paid_daystoship_to, shop.shopname as shopname, shop.address as pickup_address,
						rider.rider_name, rider.rider_platenum, rider.rider_conno, c.id as app_sales_id, c.date_shipped as shipped_date, c.payment_notes as p_notes,
						c.delivery_info as shippingpartner, c.delivery_ref_num as shipping_ref, c.delivery_notes as shipping_note,
						a.latitude as lati, a.longitude as longi, c.delivery_amount as shipping_cost, c.delivery_imgurl as pickedup_photo, c.delivery_imgurl_2 as shipped_photo,
	...
	public function modified_orders($reference_num, $sys_shop) {
		$query = "SELECT * FROM app_sales_order_details
		WHERE sys_shop = ? AND reference_num = ?" ;
		...
		$details = $this->db2->query($query, $bind_data)->row_array();

		if(empty($details)){
			return null;			
		}
		else{
			$query = "SELECT * FROM app_order_details_modified
			WHERE order_id = ?" ;
	...
	public function getSalesOrder_modify($reference_num, $sys_shop) {

		$sql = "SELECT a.*, b.latitude, b.longitude, b.notes as order_notes, c.toktok_userid, d.delivery_amount as delfee FROM app_sales_order_details AS a
		LEFT JOIN app_order_details AS b ON a.reference_num = b.reference_num
		LEFT JOIN app_customers AS c ON a.user_id = c.user_id
		LEFT JOIN app_order_details_shipping AS d ON a.reference_num = d.reference_num AND a.sys_shop = d.sys_shop
		WHERE a.sys_shop = ? AND a.reference_num = ?";
		$bind_data = array(
			$sys_shop,
			$reference_num
	...
	public function refundOrder_Qty($reference_num, $sys_shop) {
		$branchid = $this->get_branchname_id($reference_num, $sys_shop);

		$sql = "SELECT * FROM app_sales_order_details WHERE sys_shop = ? AND reference_num = ?";
		$bind_data = array(
			$sys_shop,
			$reference_num
		);

		$salesOrder = $this->db->query($sql, $bind_data)->row_array();

		$sql = "SELECT * FROM app_sales_order_logs WHERE order_id = ? AND status > 1";
		$bind_data = array(
			$salesOrder['id']
		);

		$salesOrder_logs = $this->db->query($sql, $bind_data)->result_array();

		foreach($salesOrder_logs as $row){
			$sql = "SELECT * FROM sys_products_invtrans_branch WHERE product_id = ?";
			$bind_data = array(
				$row['product_id']
			);

			$qty_logs = $this->db->query($sql, $bind_data)->row_array();
			...
			$prod_details = $this->db->query($sql, $bind_data)->row_array();

			$sql = "INSERT INTO sys_products_invtrans (id, branchid, product_id, quantity, type, username, date_created, enabled)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
			$bind_data = array(
				null,
				$branchid,
			...
			$sql = "UPDATE sys_products_invtrans_branch SET no_of_stocks = ? WHERE branchid = ? AND product_id = ?";
			$bind_data = array(
				$qty_logs['no_of_stocks']+$row['quantity'],
				$branchid,
				$row['product_id']
			);
			...
			$bind_data = array(
				$prod_details['no_of_stocks']+$row['quantity'],
				$row['product_id']
			);
    ...
    public function order_table($sys_shop){
		...
		foreach( $query->result_array() as $row ) {  // preparing an array for table tbody
			...
			$nestedData[] = str_replace($special_upper, $special_format, $row["name"]);
            ...
            if($sys_shop == 0) {
				$nestedData[] =
					'<div class="dropdown">
						...
						<div class="dropdown-menu" aria-labelledby="dropdown_menu_button">
							<a class="dropdown-item"  href="'.base_url('Main_orders/orders_view/'.$token.'/'.$row['sys_shop'].'-'.$row["reference_num"]).'/'.$order_status_view.'"><i class="fa fa-pencil" aria-hidden="true"></i> View</a>
						</div>
					</div>';
            }
            else {
				$nestedData[] =
					'<div class="dropdown">
						<i class="fa fa-ellipsis-v fa-lg" id="dropdown_menu_button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" aria-hidden="true"></i>
						<div class="dropdown-menu" aria-labelledby="dropdown_menu_button">
							<a class="dropdown-item" href="'.base_url('Main_orders/orders_view/'.$token.'/'.$row["reference_num"]).'/'.$order_status_view.'"><i class="fa fa-pencil" aria-hidden="true"></i> View</a>
						</div>
					</div>';
            }
	...
	public function order_table_shop($sys_shop){
		...
		if($branchid == 0){
			if($status == ""){
				...
				$sql = " SELECT a.order_id as orderid, a.date_ordered as date_ordered, a.reference_num as reference_num, a.name as name, SUM(s.total_amount) as total_amount, a.order_status as order_status, a.payment_status as payment_status, '".$sys_shop."' as sys_shop, shop.shopname as shopname, a.delivery_amount as delivery_amount,
				a.admin_drno, a.address, a.regCode, a.provCode, a.citymunCode, a.payment_method, a.exrate_n_to_php as currval, a.currency as currcode, a.conno, a.payment_date
				FROM `app_order_details` as a USE INDEX(date_ordered)
				LEFT JOIN `app_order_logs` as s ON a.order_id = s.order_id
	...
	public function order_item_table($reference_num, $sys_shop){
		...
		if($sys_shop == 0) {
			$sql=" SELECT d.shopcode, c.itemid as id, c.itemname as itemname, a.quantity as qty, a.amount  as amount, a.total_amount  as total_amount, c.Id as productid, c.otherinfo as unit, c.sys_shop as shop_id_item,
					...
					WHERE b.reference_num = ".$this->db->escape($reference_num). " AND a.status > 0";
		} else {
			$sql=" SELECT d.shopcode, c.itemid as id, c.itemname as itemname, a.quantity as qty, a.amount  as amount, a.total_amount  as total_amount, c.Id as productid, c.otherinfo as unit, c.sys_shop as shop_id_item,
				e.filename as primary_pic, a.exrate_n_to_php as currval, a.currency as currcode, f.itemname as parent_product_name
				FROM `app_order_logs` a
				...
				LEFT JOIN sys_products_images e ON a.product_id = e.product_id AND e.arrangement = 1 AND e.status = 1
				LEFT JOIN sys_products f ON c.parent_product_id = f.Id
				WHERE b.reference_num = ".$this->db->escape($reference_num). " AND a.sys_shop = ".$this->db->escape($sys_shop). "  AND a.status > 0";
		}
	...
	public function get_pending_orders_table($fromdate, $todate, $shopid = 'all', $branchid = 0, $requestData, $exportable = false)
	{
		...
		$sql = "SELECT
				...
				FROM (".$this->model_powb->view_paid_orders_with_branch().") a
	...
	public function orders_product_modify_table($data){
		$sql=" SELECT id FROM app_sales_order_details WHERE sys_shop = ? AND reference_num = ?";
		$params = array(
			$data["sys_shop"],
			$data["reference_num"]
		);
		$details = $this->db2->query($sql, $params)->row_array();

		$sql=" SELECT a.id as log_id, a.order_id, b.itemname as product_name, a.product_id, a.quantity, a.amount, a.total_amount FROM app_sales_order_logs AS a
		LEFT JOIN sys_products AS b ON a.product_id = b.Id
	 ...
	 public function save_modify_orders($data, $reference_num, $sys_shop){
		///update order data modified
		foreach($data as $row){
			if($row['quantity'] != $row['edit_quantity'] && $row['is_deleted'] == 0){
				$sql=" SELECT b.id, a.order_id FROM app_order_details AS a
				LEFT JOIN app_order_logs AS b ON a.order_id = b.order_id
				WHERE a.reference_num = ? AND b.sys_shop = ? AND b.product_id = ?";
				...
				$app_order_logs = $this->db2->query($sql, $params)->row_array();

				$new_total_amount = $row['edit_quantity'] * $row['amount'];
				/// update app order logs, per product
				$sql="UPDATE app_order_logs SET quantity = ?, total_amount = ? WHERE id = ? AND product_id = ?";
				...
				/// update app order details total amount
				$sql="UPDATE app_order_details SET total_amount = (SELECT SUM(total_amount) FROM app_order_logs WHERE order_id = ?  AND status > 0) WHERE reference_num = ?";

				$params = array(
					$app_order_logs['order_id'],
					$reference_num
				);

				$this->db->query($sql, $params);

				/// update app sales order logs, per product
				$sql="UPDATE app_sales_order_logs SET quantity = ?, total_amount = ? WHERE id = ? AND product_id = ?";
				...
				$this->db->query($sql, $params);
				/// update app sales order details total amount
				$sql="UPDATE app_sales_order_details SET total_amount = (SELECT SUM(total_amount) FROM app_sales_order_logs WHERE order_id = ? AND status > 0) WHERE sys_shop = ? AND reference_num = ?";
				...
				$refunded_qty = abs($row['quantity'] - $row['edit_quantity']);
				$refunded_amt = $refunded_qty * $row['amount'];

				$sql="INSERT INTO app_order_details_modified (id, order_id, logs_id, product_id, product_name, quantity, amount, total_amount, item_status, date_created, status) 
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
				$data = array(
					null, 
					$row['order_id'],
				...
				$this->refundOrderQty_modifiedOrder($reference_num, $sys_shop, $dataArr);
			}
		}

	 }

	 public function delete_modify_orders($data, $reference_num, $sys_shop){
		 ///delete order data modified
		foreach($data as $value){
			if($value['is_deleted'] == 1){
				...
				$sql="UPDATE app_order_logs SET status = ? WHERE id = ? AND product_id = ?";
				...
				/// update app sales order logs, per product
				$sql="UPDATE app_sales_order_logs SET status = ? WHERE id = ? AND product_id = ?";
	 ...
	 public function refundOrderQty_modifiedOrder($reference_num, $sys_shop, $args) {

		$branchid = $this->get_branchname_id($reference_num, $sys_shop);

		$sql = "SELECT * FROM sys_products_invtrans_branch WHERE branchid = ? AND product_id = ?";
		$bind_data = array(
			$branchid,
			$args['product_id']
		);
		...
		$sql = "UPDATE sys_products_invtrans_branch SET no_of_stocks = ? WHERE branchid = ? AND product_id = ?";
		$bind_data = array(
			$qty_logs['no_of_stocks']+$args['quantity'],
			$branchid,
			$args['product_id']
		);
		...
		$sql = "UPDATE sys_products SET no_of_stocks = ? WHERE Id = ?";
		$bind_data = array(
			$prod_details['no_of_stocks']+$args['quantity'],
			$args['product_id']
		);
	...
	public function batch_delivery_confirmed_orders($id)
	{

		$sql = "UPDATE app_sales_order_details SET isconfirmed = 1, date_confirmed = ? WHERE id = ?";
	...
	public function get_app_order_details_order_id($reference_num) {
		$query=" SELECT order_id FROM app_order_details WHERE reference_num = ?";
		$params = array($reference_num);
		return $this->db->query($query, $params)->row_array();
	}
	public function get_app_sales_order_details_reference_num($id) {
		$query=" SELECT reference_num FROM app_sales_order_details WHERE id = ?";
		$params = array($id);
		return $this->db->query($query, $params)->row_array();
	}
	public function getProductByOrderId($order_id) {
		$query=" SELECT product_id FROM app_order_logs WHERE order_id = '?' AND status = 1";
		$params = array($order_id);
		return $this->db->query($query, $params)->row_array();
	}
	public function getItemNameByProdId($product_id) {
		$query=" SELECT itemname FROM sys_products WHERE Id = '?' AND enabled != 0";
		$params = array($product_id);
		return $this->db->query($query, $params)->row_array();
	}

	public function confirmed_orders_table($sys_shop){
		// storing  request (ie, get/post) global array to a variable
		$_record_status       = $this->input->post('_record_status');
		$_name 			      = $this->input->post('_name');
		$status 		      = $this->input->post('status');
		...
		foreach( $query->result_array() as $row ) {  // preparing an array for table tbody
			...
			$nestedData[] = "<input type='checkbox' class='checkbox_perprod' id='checkbox_perprod' name='checkbox_perprod' value='".$row["orderid"]."'".$style." />";
	...
	public function confirmed_orders_table_shop($sys_shop){
		// storing  request (ie, get/post) global array to a variable
		$_record_status    = $this->input->post('_record_status');
		$_name 			   = $this->input->post('_name');
		$status 		   = $this->input->post('status');
