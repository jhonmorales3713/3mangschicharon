...
class Model_user_list extends CI_Model {
	...
	public function create_user($status,$password,$username,$avatar,$functions)
	{
		...
		$ac_reclaimed_vouchers_list_view = $this->input->post('ac_reclaimed_vouchers_list_view');
		$ac_reclaimed_vouchers_list_view = ($ac_reclaimed_vouchers_list_view) ? 1 : 0;
		...
		$ac_products_category_view = $this->input->post('ac_products_category_view');
		$ac_products_category_view = ($ac_products_category_view) ? 1 : 0;
		...
		$shop_popup_view = $this->input->post('shop_popup_view');
		$shop_popup_view = ($shop_popup_view) ? 1 : 0;
		...
		//mystery coupon
		$pr_mystery_coupon_view = $this->input->post('pr_mystery_coupon_view');
		$pr_mystery_coupon_view = ($pr_mystery_coupon_view) ? 1 : 0;
		//campaign type
		$pr_campaign_type_view = $this->input->post('pr_campaign_type_view');
		$pr_campaign_type_view = ($pr_campaign_type_view) ? 1 : 0;
		//shipping fee discount
		$pr_sfd_view = $this->input->post('pr_sfd_view');
		$pr_sfd_view = ($pr_sfd_view) ? 1 : 0;
		...
		$shop_mcr_sch_view = $this->input->post('shop_mcr_sch_view');
		$shop_mcr_sch_view = ($shop_mcr_sch_view) ? 1 : 0;
		...
		if ($queries->num_rows() > 0) {
			foreach ($queries->result() as $row) {
				...
				if ($ac_products_wfa_view == 1) {
					$content_nav_href_string = 'Products Waiting for Approval'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($ac_products_apr_view == 1) {
					$content_nav_href_string = 'Products Approved'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($ac_products_dec_view == 1) {
					$content_nav_href_string = 'Products Declined'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($ac_products_verified_view == 1) {
					$content_nav_href_string = 'Products Verified'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}

				if ($ac_products_category_view == 1) {
					$content_nav_href_string = 'Products Main Category'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($shop_popup_view == 1) {
					$content_nav_href_string = 'Shop Pop Up Image'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($shop_mcr_sch_view == 1) {
					$content_nav_href_string = 'Shop MCR Scheduling';
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($ac_reclaimed_vouchers_list_view == 1) {
					$content_nav_href_string = 'Reclaimed Vouchers'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($ac_settings_category_view == 1) {
					$content_nav_href_string = 'Product Categories'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($pr_product_promotion_view == 1) {
					$content_nav_href_string = 'Piso Day'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}

				if ($pr_mystery_coupon_view == 1) {
					$content_nav_href_string = 'Discount Promo'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($pr_sfd_view == 1) {
					$content_nav_href_string = 'Shipping Fee Discount'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($pr_campaign_type_view == 1) {
					$content_nav_href_string = 'Campaign Type'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
		...
		$query="INSERT INTO sys_users (active, password, username, avatar, functions, access_nav, access_content_nav, failed_login_attempts, tips, first_login, code_isset, login_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		return $this->db->query($query,$argument);
	}

	public function update_user($username, $password, $avatar, $functions, $id)
	{
		$sql = "SELECT `main_nav_id`, `main_nav_href` FROM `cp_main_navigation` WHERE `enabled` > 0";
		$queries = $this->db->query($sql);
		...
		if ($queries->num_rows() > 0) {
			foreach ($queries->result() as $row) {
				...
				if ($ac_vouchers_list_view == 1) {
					$content_nav_href_string = 'Vouchers List'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
				...
				if ($ac_ps_view == 1) {
					$content_nav_href_string = 'Online Store Sessions'; //get reference in cp_content_navigation->cn_name
					if ($content_nav_href_string == $row->cn_name) {
						$array_content_nav_id[] = $row->id;
					} 
				}
	...
	{
		$sql = "UPDATE sys_users SET active = 0 WHERE id = ?";
	...
	public function user_list_table($filters, $requestData, $exportable = false)
	{
		...
		if(ini() == 'toktokmall'){
			...
			if ($this->loginstate->get_access()['users']['disable'] == 1) {
				$actions .= '
			...
			if ($this->loginstate->get_access()['users']['delete'] == 1) {
				$actions .= '
