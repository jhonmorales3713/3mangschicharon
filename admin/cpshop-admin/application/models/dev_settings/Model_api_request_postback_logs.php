...
class Model_api_request_postback_logs extends CI_Model {
	public $app_db;

	public function api_request_postback_logs_table($exportable = false){
		...
		$sql = "SELECT * FROM sys_apirequest_postlog
			    WHERE status = '1'AND DATE(date_created) BETWEEN ".$this->db->escape($date_from)." AND ".$this->db->escape($date_to)."";

		// getting records as per search parameters

		if($_details != ""){
			$sql.=" AND data_request LIKE '%".$this->db->escape_like_str($_details)."%'";
		}
		...
		foreach( $query->result_array() as $row ) {  // preparing an array for table tbody
			...
			$str =  $row["data_request"];
			$nestedData[] = (!$exportable) ? '<textarea disabled readonly class = "form-control" rows = "4" cols = "30">'.stripcslashes($str).'</textarea>':stripcslashes($str);
			$nestedData[] = $row["type"];
			$data[] = $nestedData;
		}
	...
	public function get_type_apirequest_postlog(){
        $query="SELECT * FROM sys_apirequest_postlog
               GROUP BY type";
