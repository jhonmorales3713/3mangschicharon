...
class Model_sales_report extends CI_Model {
  ...
  public function get_sales_reports_data($fromdate,$todate,$shopid,$branchid,$filtertype, $pmethodtype, $requestData, $exportable = false){
    $paid = $this->sum_all_sales_report($fromdate,$todate,$shopid,'paid');
    $unpaid = $this->sum_all_sales_report($fromdate,$todate,$shopid,'unpaid');
    ...
    IF(`c`.`status` = 1, c.refund_amount, '') AS `amount_refunded`
FROM
    (
        (
    ...
    LEFT JOIN(
        ...
        WHERE
            det.is_checked = 1 AND summary.status = 1
        GROUP BY
            summary.refnum,
            det.sys_shop,
            det.branchid
...
ON
    (
        `a`.`reference_num` = `c`.`refnum` AND `c`.`sys_shop` = `a`.`sys_shop` AND `c`.`branchid` = 0
    )
    )
WHERE
    `a`.`status` = 1 AND `a`.`payment_status` = 1 AND !(
        `a`.`reference_num` IN(
        SELECT
...
SELECT
    ...
    IF(`f`.`status` = 1, f.refund_amount, '') AS `amount_refunded`
FROM
    (
        (
            (
                ...
                ON
                    (
                        `b`.`mainshopid` = `c`.`sys_shop` AND `a`.`orderid` = `c`.`reference_num`
                    )
                )
...
ON
    (
        `f`.`refnum` = `c`.`reference_num` AND `f`.`sys_shop` = `c`.`sys_shop` AND `f`.`branchid` = `a`.`branchid`
    )
    )
WHERE
    `a`.`status` = 1 AND `c`.`status` = 1 AND `c`.`payment_status` = 1
HAVING total_amount != amount_refunded
UNION
SELECT
    `a`.`id` AS `order_id`,
...
WHERE
    `a`.`status` = 1 AND `a`.`payment_status` = 1
ORDER BY order_id";
    $sql = "SELECT tbl.`name`, tbl.payment_date, tbl.date_ordered, tbl.reference_num, tbl.total_amount, 1 as payment_status, tbl.shopname, tbl.branchname FROM (".$pre_sql.") AS tbl WHERE DATE(tbl.payment_date) BETWEEN $fromdate AND $todate";
  ...
  public function get_sales_reports_chart_data($fromdate,$todate,$shopid,$filtertype){
    $fromdate = $this->db2->escape($fromdate);
    $todate = $this->db2->escape($todate);
  ...
  public function sum_all_sales_report($fromdate,$todate,$shopid,$filtertype){
    $fromdate = $this->db2->escape($fromdate);
    $todate = $this->db2->escape($todate);
    $sql = "SELECT SUM(total_amount) as totalamount, DATE(date_ordered) as trandate
      FROM app_sales_order_details WHERE status = 1
