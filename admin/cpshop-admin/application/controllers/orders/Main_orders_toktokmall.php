...
class Main_orders_toktokmall extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('orders/model_orders_toktokmall');
    ...
    public function views_restriction($content_url)
    {
        ...
        if (in_array($content_url, $url_content_arr) == false) {
            if ($is_seller == 1 && $content_url == 'Main_orders/orders_merchant/') {
    ...
    public function delete_modal_confirm()
    {
        ...
        if ($delete_id === 0) {
            ...
            $query = $this->model_orders_toktokmall->delete_modal_confirm($delete_id);
    ...
    public function disable_modal_confirm()
    {
        ...
        if ($disable_id > 0 && $record_status > 0) {
            $query = $this->model_orders_toktokmall->disable_modal_confirm($disable_id, $record_status);
    ...
    public function orders($token = '')
    {
        ...
        if($this->loginstate->get_access()['overall_access'] == 1 || $this->loginstate->get_access()['transactions']['view'] == 1 || $is_seller == 1) {
            ...
            $data_admin = array(
                ...
                'shopid'              => $this->model_orders_toktokmall->get_sys_shop($member_id),
                'shopcode'            => $this->model_orders_toktokmall->get_shopcode($member_id),
                'shops'               => $this->model_orders_toktokmall->get_shop_options(),
                'regions'             => $this->model_orders_toktokmall->get_regions()->result_array(),
                'provinces'           => $this->model_orders_toktokmall->get_provinces()->result_array(),
                'citymuns'            => $this->model_orders_toktokmall->get_citymuns()->result_array(),
                'branchid' 		      => $this->session->userdata('branchid')
            );

            $this->load->view('includes/header', $data_admin);
            $this->load->view('orders/orders_toktokmall', $data_admin);
        }else{
    ...
    public function orders_view($token = '', $ref_num, $order_status_view)
    {
        ...
        if($this->loginstate->get_access()['overall_access'] == 1 || $this->loginstate->get_access()['transactions']['view'] == 1 || $this->loginstate->get_access()['pending_orders']['view'] == 1 || $this->loginstate->get_access()['paid_orders']['view'] == 1 || $this->loginstate->get_access()['readyforprocessing_orders']['view'] == 1 || $this->loginstate->get_access()['processing_orders']['view'] == 1 || $this->loginstate->get_access()['readyforpickup_orders']['view'] == 1 || $this->loginstate->get_access()['bookingconfirmed_orders']['view'] == 1 || $this->loginstate->get_access()['fulfilled_orders']['view'] == 1 || $this->loginstate->get_access()['shipped_orders']['view'] == 1 || $this->loginstate->get_access()['returntosender_orders']['view'] == 1) {
            $member_id = $this->session->userdata('sys_users_id');
            $sys_shop = $this->model_orders_toktokmall->get_sys_shop($member_id);
            ...
            if($sys_shop == 0) {
                ...
                // $date_ordered  = $this->model_orders_toktokmall->get_app_order_details_date_ordered($reference_num);
                // $prev_order    = $this->model_orders_toktokmall->get_prev_orders($reference_num, $date_ordered, $order_status_view);
                // $next_order    = $this->model_orders_toktokmall->get_next_orders($reference_num, $date_ordered, $order_status_view);
                ...
                if($sys_shop == 0){
                    $get_vouchers = $this->model_orders_toktokmall->get_vouchers($reference_num);
                }else{
                    $get_vouchers = $this->model_orders_toktokmall->get_vouchers_shop($reference_num, $sys_shop);
                }
            }else{
                $reference_num = $ref_num;
                // $date_ordered  = $this->model_orders_toktokmall->get_app_order_details_date_ordered($reference_num);
                // $prev_order    = $this->model_orders_toktokmall->get_prev_orders_per_shop($reference_num, $sys_shop, $date_ordered, $order_status_view);
                // $next_order    = $this->model_orders_toktokmall->get_next_orders_per_shop($reference_num, $sys_shop, $date_ordered, $order_status_view);
            ...
            $row            = $this->model_orders_toktokmall->orders_details($reference_num, $sys_shop);
            $refcode        = $this->model_orders_toktokmall->get_referral_code($row['reference_num']);
            $branch_details = $this->model_orders_toktokmall->get_branchname_orders($reference_num, $sys_shop)->row();
            $order_items    = $this->model_orders_toktokmall->order_item_table_print($reference_num, $sys_shop);
            $refundedOrder  = $this->model_orders_toktokmall->get_refundedOrder($reference_num, $sys_shop)->result_array();
            
            if($sys_shop != 0){
                $mainshopname   = $this->model_orders_toktokmall->get_mainshopname_all($sys_shop)->row()->shopname;
                $shop_status    = $this->model_orders_toktokmall->get_mainshopname_all($sys_shop)->row()->status;
            ...
            $orders_history = $this->model_orders_toktokmall->orders_history($row['order_id']);
            if(!empty($row['app_sales_id'])){
                $orders_history_sales = $this->model_orders_toktokmall->orders_history_sales($row['app_sales_id']);
            }else{
            ...
            $data_admin = array(
                ...
                'shopid'              => $this->model_orders_toktokmall->get_sys_shop($member_id),
                'shopcode'            => $this->model_orders_toktokmall->get_shopcode($member_id),
                'partners'            => $this->model_orders_toktokmall->get_partners_options(),
                'partners_api_isset'  => $this->model_orders_toktokmall->get_partners_options_api_isset(),
                'payments'            => $this->model_orders_toktokmall->get_payments_options(),
                ...
                'mainshopid'          => $sys_shop,
                'branch_count'        => count($this->model_orders_toktokmall->get_all_branch($sys_shop)->result()),
                'url_ref_num'         => $ref_num,
                'prev_order'          => '',
                'next_order'          => '',
                ...
                'getOrderLogs'        => $this->model_orders_toktokmall->getOrderLogs_orderid($row['orderid'])->result_array()
            );

            $this->load->view('includes/header', $data_admin);
            $this->load->view('orders/orders_view_toktokmall', $data_admin);
        }else{
            $this->load->view('error_404');
        }
    }
    ...
    {   
        ...
        if(!empty($mainshopid) AND !empty($remarks) AND !empty($reference_num)){
            if($branchid != "" || $branchid != null || !empty($branchid)){
                $get_branch_details = $this->model_orders_toktokmall->get_branch_details_id($branchid)->row();
                $items              = $this->model_orders_toktokmall->listShopItems($reference_num, $mainshopid);
                $branch             = $get_branch_details->branchname;

                foreach($items as $row){
                    $details = $this->model_orders_toktokmall->checkinvQty_fromBranch($row['productid'], $branchid, $mainshopid)->row_array();
                ...
                foreach($items as $row){
                    $details = $this->model_orders_toktokmall->checkinvQty_fromBranch($row['productid'], 0, $mainshopid)->row_array();
                    if(!empty($details['no_of_stocks'])){
                        if($row['qty'] > $details['no_of_stocks']){
                            $left         = ($details['no_of_stocks'] == 0) ? "out of stock" : $details['no_of_stocks']." unit/s left";
                            $message_str .= $details['itemname']." - ".$left."<br />";
    ...
    {   
        ...
        if(!empty($mainshopid) AND !empty($remarks) AND !empty($reference_num)){
            $this->model_orders_toktokmall->reassign_branch($branchid, $reference_num, $remarks, $mainshopid, $prev_branchid);
            $this->model_orders_toktokmall->reassign_date($reference_num, $mainshopid);
            ...
            if($branchid != "" || $branchid != null || !empty($branchid)){
                ...
                $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $mainshopid);

                $items   = $this->model_orders_toktokmall->listShopItems($reference_num, $mainshopid);
                foreach($items as $row){
                    $this->model_orders_toktokmall->updateInv_fromBranch($row['productid'], $row['qty'], $prev_branchid, $mainshopid);
                    $this->model_orders_toktokmall->updateInv_toBranch($row['productid'], $row['qty'], $branchid, $mainshopid);
                }

                if($prev_branchid != 0){
                    $this->model_orders_toktokmall->update_branch_pending_orders($prev_branchid, '-');
                }

                if($branchid != 0){
                    $this->model_orders_toktokmall->update_branch_pending_orders($branchid, '+');
                }
                
                $branch = $get_branch_details->branchname;
                $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order has been transferred to '.$get_branch_details->branchname.' Branch.', 'Re Assign', $this->session->userdata('username'), date('Y-m-d H:i:s'));
            }else{
                $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $mainshopid);
                ...
                foreach($items as $row){
                    $this->model_orders_toktokmall->updateInv_fromBranch($row['productid'], $row['qty'], $prev_branchid, $mainshopid);
                    $this->model_orders_toktokmall->updateInv_toBranch($row['productid'], $row['qty'], 0, $mainshopid);
                }
                ...
                $branch = "Main";
                $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order has been transferred to Main Branch.', 'Re Assign', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    {
        $mainshopid = sanitize($this->input->post('mainshopid'));
        $shopbranches = $this->model_orders_toktokmall->get_all_branch($mainshopid);
    ...
    public function payOrder()
    {
        ...
        if ($this->input->post('f_payment_ischecked') == TRUE && $this->form_validation->run() == FALSE) {
            ...
            if($this->input->post('f_payment') != 'Others' && $this->input->post('f_payment_ischecked') == TRUE) {
                $payment_partner = $this->model_orders_toktokmall->get_payment_type($this->input->post('f_payment'))['description'];
            } else {
            ...
            $isPaid           = $this->model_orders_toktokmall->getPaidOrder($reference_number)->num_rows();
    ...
    function preparePaymentEmail($reference_num, $shop) 
    {
        ...
        //Prepare email
        $items = $this->model_orders_toktokmall->listShopItems($reference_num, $shop);

        $shopItems = array();
        foreach($items as $item){
          $shopDetails = $this->model_orders_toktokmall->read_shop($shop);
          if(empty($shopItems[$shop])) {

            if(count($this->model_orders_toktokmall->get_daystoship($reference_num, $shop)) > 0){
                $shippingdts = $this->model_orders_toktokmall->get_daystoship($reference_num, $shop);    
            }else{
        ...
        $orderDetails = $this->model_orders_toktokmall->orders_details($reference_num, $shop);
    ...
    function transferOrderEmailBranch($reference_num, $shop, $branchid)
    {
        ...
        $orderDetails["payment_status"] = "Paid";
        $orderDetails["payment_method"] = "Paid to Seller";
        $orderDetails['branchEmail'] = $this->model_orders_toktokmall->get_branch_details_id($branchid)->row()->email;
        $orderDetails['branchname'] = $this->model_orders_toktokmall->get_branch_details_id($branchid)->row()->branchname;
    ...
    public function processOrder()
    {
        ...
        $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged($sys_shop, $reference_num, 'p');
        ...
        if($checkIfOrderChanged == 1){
            ...
            $success    = $this->model_orders_toktokmall->processOrder($reference_num, $sys_shop);
            $items      = $this->model_orders_toktokmall->listShopItems($reference_num, $sys_shop);
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order is being prepared for shipping by the seller.', 'Process Order', $this->session->userdata('username'), date('Y-m-d H:i:s'));

            $shopItems = array();
            foreach($items as $item){
                $shopDetails = $this->model_orders_toktokmall->read_shop($sys_shop);
                if(empty($shopItems[$sys_shop])) {

                    if(count($this->model_orders_toktokmall->get_daystoship($reference_num, $sys_shop)) > 0){
                        $shippingdts = $this->model_orders_toktokmall->get_daystoship($reference_num, $sys_shop);    
                    }else{
                        $shippingdts['daystoship'] = $shopDetails["daystoship"];
                        $shippingdts['daystoship_to'] = $shopDetails["daystoship"];
                    }
                    ...
                    $shopItems[$sys_shop] = array(
                        "shopname" => $shopDetails["shopname"],
                        "branchname" => $this->model_orders_toktokmall->get_branchname($reference_num, $sys_shop),
                        "shopcode" => $shopDetails["shopcode"],
            ...
            $order = $this->model_orders_toktokmall->orders_details($reference_num, $sys_shop);
    ...
    public function readyPickupOrder()
    {
        ...
        if($rp_pickup_address != ''){
            ...
            $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged($sys_shop, $reference_num, 'po');
            ...
            {
                $this->model_orders_toktokmall->updateLongLatCustomer($reference_num, $loc_latitude, $loc_longitude, $rp_notes);
                $salesOrder = $this->model_orders_toktokmall->getSalesOrder_book($reference_num, $sys_shop);

                if($rp_shipping_partner != ""){
                    if($rp_shop_id == 0) {
                        $senderDetails = $this->model_orders_toktokmall->get_branch_details_id($rp_branch_id)->row_array();
                        $senderName    = $senderDetails['branchname'];
                        $senderMobile  = $senderDetails['mobileno'];
                        $this->model_orders_toktokmall->updatePickupAddressBranch($rp_branch_id, $rp_pickup_address);
                    }else{
                        $senderDetails = $this->model_orders_toktokmall->get_shop_details_id($sys_shop)->row_array();
                        $senderName    = $senderDetails['shopname'];
                        $senderMobile  = $senderDetails['mobile'];
                        $this->model_orders_toktokmall->updatePickupAddressShop($rp_shop_id, $rp_pickup_address);
                    ...
                    $data = array(
                        ...
                        'hash'             => $this->model_orders_toktokmall->getDetailsfromShipping($sys_shop, $reference_num)['hash'],
                    ...
                    $success         = $this->model_orders_toktokmall->readyPickupOrder_toktok($reference_num, $sys_shop, $price, $deliveryId, $rp_shipping_partner, $shareLink, $referralCode);
                    ...
                    $success = $this->model_orders_toktokmall->readyPickupOrder($reference_num, $sys_shop);
                ...
                $items   = $this->model_orders_toktokmall->listShopItems($reference_num, $sys_shop);
                $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order is ready for pickup. <u><a href='.$shareLink.' target="_blank"><small>Delivery Link</small></a></u>', 'Ready for Pickup', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    public function cancelOrder()
    {
        ...
        $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged($sys_shop, $reference_num, 'rp');
        ...
        if($checkIfOrderChanged == 1){
            ...
            $success = $this->model_orders_toktokmall->cancelOrder($reference_num, $sys_shop);
        
            $items   = $this->model_orders_toktokmall->listShopItems($reference_num, $sys_shop);
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Booking has been cancelled.', 'Cancel Booking', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    public function getDeliveryCancellationCategories(){
        ...
        $CustomerDetails = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
    ...
    public function bookingConfirmOrder()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $success = $this->model_orders_toktokmall->bookingConfirmOrder($reference_num, $sys_shop);
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Booking of order is confirmed.', 'Booking Confirmed', $this->session->userdata('username'), date('Y-m-d H:i:s'));

            if($this->input->post('bc_rider_ischecked') == TRUE) {
                $this->model_orders_toktokmall->tagRider($this->input->post(), $success);
            }
            ...
            $checkbranchOrder        = $this->model_orders_toktokmall->checkbranchOrder($reference_num, $sys_shop);
    ...
    public function FulfilledOrder()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged_fulfilled($sys_shop, $reference_num);
            ...
            if($checkIfOrderChanged == 1){
                ...
                // if($this->input->post('f_shipping_ischecked') == TRUE) {
                    $this->model_orders_toktokmall->tagShipping($reference_num, $this->input->post(), $sys_shop);
                // }

                $success = $this->model_orders_toktokmall->FulfilledOrder($reference_num, $sys_shop);
                $branchid_if_assigned = $this->model_orders_toktokmall->get_branchname_id($reference_num, $sys_shop);

                if($branchid_if_assigned != 0){
                    $this->model_orders_toktokmall->update_branch_pending_orders($branchid_if_assigned, '-');
                }

                if($this->input->post('mf_rider_ischecked') == TRUE) {
                    $this->model_orders_toktokmall->mf_tagRider($this->input->post(), $success);
                }

                $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
                $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order is being delivered.', 'Mark as Fulfilled', $this->session->userdata('username'), date('Y-m-d H:i:s'));
                ...
                if($this->input->post('f_shipping') != 'Others'){
                    $shippingname = $this->model_orders_toktokmall->get_shipping_partner($this->input->post('f_shipping'));
                }
                else{
    ...
    public function returntosenderOrder()
    {
        ...
        $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged($sys_shop, $reference_num, 'f');
        ...
        if($checkIfOrderChanged == 1){
            ...
            $success = $this->model_orders_toktokmall->returntosenderOrder($reference_num, $sys_shop);
            $items   = $this->model_orders_toktokmall->listShopItems($reference_num, $sys_shop);
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order has been returned to sender.', 'Return to Sender', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    public function redeliverOrder()
    {
        ...
        $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged($sys_shop, $reference_num, 'rs');
        ...
        if($checkIfOrderChanged == 1){
            ...
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $success = $this->model_orders_toktokmall->redeliverOrder($reference_num, $sys_shop, $salesOrder->id);
            $items   = $this->model_orders_toktokmall->listShopItems($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order will re-deliver.', 'ReDeliver Order', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    public function shippedOrder()
    {
        ...
        if($checkIfOrderChanged == 1){
            ...
            $success = $this->model_orders_toktokmall->shippedOrder($reference_num, $sys_shop);
            $items   = $this->model_orders_toktokmall->listShopItems($reference_num, $sys_shop);
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order has been successfully shipped.', 'Mark as Shipped', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    public function confirmedOrder()
    {
        ...
        $checkIfOrderChanged = $this->model_orders_toktokmall->checkIfOrderChanged($sys_shop, $reference_num, 's');
        ...
        if($checkIfOrderChanged == 1){
            ...
            $success = $this->model_orders_toktokmall->confirmedOrder($reference_num, $sys_shop);
            $salesOrder = $this->model_orders_toktokmall->getSalesOrder($reference_num, $sys_shop);
            $this->model_orders_toktokmall->addOrderHistory($salesOrder->id, 'Order has been successfully confirmed.', 'Order Confirmed', $this->session->userdata('username'), date('Y-m-d H:i:s'));
    ...
    public function order_table()
    {
        ...
        $sys_shop = $this->model_orders_toktokmall->get_sys_shop($member_id);
        if($sys_shop == 0){
            $query = $this->model_orders_toktokmall->order_table($sys_shop);
        }else{
            $query = $this->model_orders_toktokmall->order_table_shop($sys_shop); 
        }
    ...
    public function order_item_table()
    {
        ...
        $query = $this->model_orders_toktokmall->order_item_table($reference_num, $sys_shop);
    ...
    public function export_orders()
    {
        ...
        if($sys_shop == 0){
            $query = $this->model_orders_toktokmall->order_table_export($sys_shop);
        }else{
            $query = $this->model_orders_toktokmall->order_table_shop_export($sys_shop); 
        }
        ...
        foreach($query as $row){
            ...
            $branchname = $this->model_orders_toktokmall->get_branchname($row["reference_num"], $row['sys_shop']);
    ...
    public function export_orders_backup()
    {
        ...
        $getShippingData    = $this->model_orders_toktokmall->getShippingData($_name, $date_from_2, $date_to_2)->result_array();
        $getShippingDataArr = [];
        $getShopData        = $this->model_orders_toktokmall->getShopData()->result_array();
		$getShopDataArr     = [];
        ...
        foreach($query as $row){
            ...
            if($row['sys_shop'] != 0) {
				$voucher_total_amount = $this->model_orders_toktokmall->get_vouchers_total_shop($row["reference_num"], $row["sys_shop"])->total_voucher_amount;
            ...
            $resultArray = array(
                ...
                '11' => $shopname,
                '12' => $branchname,
                '13' => $this->model_orders_toktokmall->get_citymun($row['citymunCode'])
            );
    ...
    public function print_order($token = '', $ref_num)
    {
        ...
        if($this->loginstate->get_access()['overall_access'] == 1 || $this->loginstate->get_access()['transactions']['view'] == 1 || $this->loginstate->get_access()['pending_orders']['view'] == 1 || $this->loginstate->get_access()['paid_orders']['view'] == 1 || $this->loginstate->get_access()['readyforprocessing_orders']['view'] == 1 || $this->loginstate->get_access()['processing_orders']['view'] == 1 || $this->loginstate->get_access()['readyforpickup_orders']['view'] == 1 || $this->loginstate->get_access()['bookingconfirmed_orders']['view'] == 1 || $this->loginstate->get_access()['fulfilled_orders']['view'] == 1 || $this->loginstate->get_access()['shipped_orders']['view'] == 1 || $this->loginstate->get_access()['returntosender_orders']['view'] == 1) {
            ...
            if($sys_shop != 0){
                $mainshopname = $this->model_orders_toktokmall->get_mainshopname($sys_shop)->row()->shopname;
            }else{
                $mainshopname = get_company_name();
            }
