...
class Settings_province extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('setting/Model_settings_province');
	}
    ...
    public function delete_modal_confirm(){
        ...
        if ($delete_id > 0) {
            $query = $this->Model_settings_province->delete_modal_confirm($delete_id);

            $prov_data = $this->Model_settings_province->get_data($delete_id)->row();
		    $remarks = $prov_data->provDesc." has been successfully deleted";

            if ($query == 1) {
                $data = array("success" => 1, 'message' => "Record deleted successfully!");
                $this->audittrail->logActivity('Province', $remarks, 'deleted', $this->session->userdata('username'));
            }else{
    ...
    public function disable_modal_confirm(){
        ...
        $prov_data = $this->Model_settings_province->get_data($disable_id)->row();
		$remarks = $prov_data->provDesc." has been successfully ".$record_text;      

        if ($disable_id > 0 && $record_status > 0) {
            $query = $this->Model_settings_province->disable_modal_confirm($disable_id, $record_status);

            if ($query == 1) {
                $data = array("success" => 1, 'message' => "Record ".$record_text." successfully!");
                $this->audittrail->logActivity('Province', $remarks, $record_text, $this->session->userdata('username'));
            }else{
                $data = array("success" => 0, 'message' => "Something went wrong, Please Try again!");
            }
        }else{
    ...
    public function view($token = '')
    {
        ...
        if ( $this->loginstate->get_access()['settings_province']['view'] == 1){
            ...
            $this->load->view('settings/province/province_list_table', $data_admin);
    ...
    public function province_list(){
        ...
        $filters = [
            ...
            '_searchby'      => sanitize($this->input->post('_searchby')),
        ];
        $query = $this->Model_settings_province->province_list($filters, $_REQUEST);
        generate_json($query);
    }

    public function export_province_list(){
        $this->isLoggedIn();
        $requestData = url_decode(json_decode($this->input->post('_search')));
        $filters = (array) json_decode($this->input->post('_filter'));
        // print_r($filters);
        // exit();
        $query = $this->Model_settings_province->province_list($filters, $requestData, true);
        // print_r($query);
        // exit();
        $fromdate = date('Y-m-d');
        $fil_arr = [
        ...
        extract($this->audittrail->get_ReportExportRemarks("", "", $fromdate, $fromdate, "Province", $fil_arr));
        // print_r($remarks);
        // exit();
        $this->audittrail->logActivity('Province', $remarks, 'export', $this->session->userdata('username'));
        ...
        $sheet->setCellValue("A6", 'Province Code');
        $sheet->setCellValue('B6', 'Province Name');
        $sheet->setCellValue('C6', 'Region Code');
        $sheet->setCellValue('D6', 'Region Name');
