...
class Settings_city extends CI_Controller {
    ...
    public function delete_modal_confirm(){
        ...
        if ($delete_id > 0) {
            $query = $this->Model_settings_city->delete_modal_confirm($delete_id);

            if ($query == 1) {
                $data = array("success" => 1, 'message' => "Record deleted successfully!");
				$this->audittrail->logActivity('City', "City $record_name has been deleted successfully.", 'delete', $this->session->userdata('username'));
            }else{
    ...
    public function disable_modal_confirm(){
        ...
        if ($disable_id > 0 && $record_status > 0) {
            $query = $this->Model_settings_city->disable_modal_confirm($disable_id, $record_status);

            if ($query == 1) {
                $data = array("success" => 1, 'message' => "Record ".$record_text." successfully!");
				$this->audittrail->logActivity('City', "City $record_name has been $record_text successfully.", $record_text, $this->session->userdata('username'));
            }else{
                $data = array("success" => 0, 'message' => "Something went wrong, Please Try again!");
            }
        }else{
    ...
    public function view($token = '')
    {
        ...
        if ( $this->loginstate->get_access()['settings_city']['view'] == 1){
            ...
            $this->load->view('settings/city/city_list_table', $data_admin);
    ...
    public function city_list(){
        ...
        $query = $this->Model_settings_city->city_list($filters, $_REQUEST);
        generate_json($query);
    }

    public function export_city_list(){
        $this->isLoggedIn();
        $requestData = url_decode(json_decode($this->input->post('_search')));
        $filters = (array) json_decode($this->input->post('_filter'));
        // print_r($filters);
        // exit();
        $query = $this->Model_settings_city->city_list($filters, $requestData, true);
        // print_r($query);
        // exit();
        $fromdate = date('Y-m-d');
        $fil_arr = [
        ...
        extract($this->audittrail->get_ReportExportRemarks("", "", $fromdate, $fromdate, "City", $fil_arr));
        // print_r($remarks);
        // exit();
        $this->audittrail->logActivity('City', $remarks, 'export', $this->session->userdata('username'));
        ...
        $sheet->setCellValue("A6", 'City Code');
        $sheet->setCellValue('B6', 'City Name');
        $sheet->setCellValue('C6', 'Province');
