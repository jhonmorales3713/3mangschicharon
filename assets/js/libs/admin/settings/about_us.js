...
$(function () {
    ...
    $("#btnsaveArrangement").click(function(){
		...
		$.ajax({
			type:'post',
			url:base_url+'admin/settings/Website_info/save_arrangement_aboutus',
    ...
    $("#btnSaveaboutus").click(function(){
        var form = $("form[name=form_aboutus]");
        var form_data = new FormData(form[0]);
        form_data.append('selectedid',selectedid);
        $.LoadingOverlay("show");
        $.ajax({
            type: 'post',
            url: base_url+'admin/settings/website_info/save_aboutus',
            data: form_data,
            contentType: false,   
            cache: false,      
            processData:false,
    ...
    $("#disable_enable_modal_confirm_btn").click(function(){
        ...
        $.ajax({
            type: 'post',
            url: base_url+'admin/settings/website_info/enable_disable_aboutus',
            data: form_data,
            contentType: false,   
            cache: false,      
            processData:false,
    ...
    $("#delete_modal_confirm_btn").click(function(){
        ...
        $.ajax({
            type: 'post',
            url: base_url+'admin/settings/website_info/delete_aboutus',
            data: form_data,
            contentType: false,   
            cache: false,      
            processData:false,
    ...
    $('#add_modal').on('show.bs.modal', function (e) {
        ...
        if(selectedid!=''){
            $.ajax({
                type: 'post',
                url: base_url+'admin/settings/website_info/get_aboutus',
                data: {'id':selectedid},
                success:function(data){
                    $("#add_update_title").html("Update Info");
            ...
            $("#add_update_title").html("Add Info");
        }
    });
	...
	function fillDatatable(){
		...
		dataTable = $('#tblaboutus').DataTable({
			"processing": false,
			destroy: true,
            rowReorder: {
			...
			"ajax":{
				type: "post",
				url:base_url+"admin/settings/Website_info/load_aboutus", // json datasource
				data: {
                        '_search':search,
                        '_status':status
				...
				complete: function(data) {
                   ...
                   // old_order = $("#tblaboutus tbody").html().replace("odd",'').replace("even",'').replace('style=""','').replace('class="" ','class=""');
                    ...
                    var data = $('#tblaboutus').DataTable().rows().data();
                    old_order=[];
                    data.each(function (value, index) {
                        old_order.push(value[1]);
                    });

                    $(".update_aboutus,.action_disable,.action_delete").click(function(){
